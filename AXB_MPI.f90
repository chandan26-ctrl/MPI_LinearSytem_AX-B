...
!****************************************************************************
      ...
      INTEGER :: COLS,NCOL,EXTRA, OFFSET,I,J,K,T,LED,IERR, INSET
      INTEGER :: STATUS(MPI_STATUS_SIZE)
      REAL :: A(N,N), B(N), C(N),L(N,N),U(N,N),D(N,N), COEFF ,Y(N),SUMM
...
!     SEND MATRIX DATA TO SLAVE PROCESSORS
      ...
      DO NP=1, NSLAVES
        ...
        CALL MPI_SEND( B,   N,    MPI_REAL,    NP, MSGTYPE, MPI_COMM_WORLD, IERR)
...
!     RECEIVE MATRIX DATA FROM THE MASTER PROCESSOR
        ...
        CALL MPI_RECV( B, N, MPI_REAL, MASTER, MSGTYPE, MPI_COMM_WORLD, STATUS, IERR )
...
! COMPUTE COLUMNS OF MAT D^-1*B(DX=B)       
        DO I=1,N
           D(I,T)=D(I,T)*B(INSET)
        END DO
